...
def GetInitialPriceSet():
    return [customer[i].c0 for i in range(N)]
...
class sfz_dynamic_model():
    ...
    def alogor1(self, H, N, K, graph=False):
        C = list()
        for i in range(H):
            select_p = 0
            selected_value = 0
    ...
    def Generator(self):
        ...
        FILE_NAME = f"{VERSION}/kmean_N{N}H{H}K{CLUSTER}.pickle"
        if not (Path(FILE_NAME).exists() and Path(FILE_NAME).stat().st_size > 0):
            print(f"Kmeans cal..")
            with open(FILE_NAME, 'wb') as f:
                self.kmeans, self.scaler = self.alogor1(H, N, CLUSTER, 0)
